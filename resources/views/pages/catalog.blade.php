    ...
    <section class="sticky top-[72px] z-40 bg-white/80 backdrop-blur-md border-b border-slate-100 py-6 px-4">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex gap-4 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto scrollbar-hide">
                ...
                @foreach(['Oli Mesin', 'Suku Cadang', 'Aksesoris'] as $cat)
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div class="group bg-white rounded-[2.5rem] p-6 border border-slate-100 shadow-sm hover:shadow-[0_20px_50px_-20px_rgba(30,58,138,0.2)] hover:-translate-y-2 transition-all duration-500 relative overflow-hidden flex flex-col h-full">
                    ...
                    <div class="absolute top-6 right-6 z-10">
                        @if($product->stock > 0)
                            <span class="bg-emerald-100 text-emerald-600 text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-tighter">Ready: {{ $product->stock }}</span>
                        @else
                            <span class="bg-red-100 text-red-600 text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-tighter">Habis</span>
                        @endif
                    </div>

                    {{-- Image Area - Linked to Detail --}}
                    ...
                    <div class="flex-1 flex flex-col">
                        ...
                        <div class="mt-4 space-y-3">
                            ...
                            <p class="text-slate-400 text-xs italic leading-relaxed line-clamp-2 text-left">
                                {{ $product->description ?: 'Kualitas original untuk menjaga durabilitas mesin kendaraan Anda.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="pt-5 flex items-center justify-between border-t border-slate-100 mt-6">
                        <div class="flex flex-col text-left">
                            <span class="text-[10px] text-slate-400 uppercase font-bold tracking-widest">Harga</span>
                            <span class="text-xl font-black text-blue-950 italic tracking-tighter">
